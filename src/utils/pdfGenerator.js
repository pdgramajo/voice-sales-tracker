...
export const generatePDF = (sales, expenses, initialBalance = 0) => {
  const cashSales = sales.filter(v => v.paymentMethod === 'efectivo').reduce((sum, v) => sum + v.amount, 0);
  const transferTotal = sales.filter(v => v.paymentMethod === 'transferencia').reduce((sum, v) => sum + v.amount, 0);
  ...
  doc.text(`Total Gastos: ${formatCurrency(totalExpenses)}`, pageWidth - 20, yPos, { align: 'right' });
  ...
  doc.text(`Transferencia: ${formatCurrency(transferTotal)}`, pageWidth - 20, yPos, { align: 'right' });
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text(`EN CAJA: ${formatCurrency(cashInDrawer)}`, pageWidth - 25, yPos + 9.5, { align: 'right' });
